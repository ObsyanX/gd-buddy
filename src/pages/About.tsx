...
function RevealSection({ children, className, delay = 0 }: { children: React.ReactNode; className?: string; delay?: number }) {
  ...
  return (
    <div
      ref={ref}
      className={cn(revealClasses, isVisible ? visibleClasses : hiddenClasses, className)}
...
const About = () => (
  <>
    ...
    <div className="min-h-screen bg-background">
          ...
          <div className="grid sm:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {FEATURES.map((f, i) => (
              <RevealSection key={f.title} delay={i * 100}>
                <Card className="border-2 transition-all duration-200 hover:-translate-y-1 hover:shadow-lg h-full">
      ...
      {/* ── FOOTER NOTE ── */}
      <RevealSection className="text-center pb-8 px-6">
